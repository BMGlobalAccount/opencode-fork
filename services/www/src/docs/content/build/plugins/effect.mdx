...
Key methods can provide an optional `validate` callback returning an Effect. It receives the proposed `Credential.Key`,
including form answers in `configuration`, before persistence. A failure rejects the connection and preserves the active
account; loading stored accounts does not call it.
...
yield* ctx.integration.transform((editor) => {
  editor.method.update({
    ...
    validate: (credential) =>
      credential.key.trim() ? Effect.void : Effect.fail(new Error("API key is required")),
  })
})
```
