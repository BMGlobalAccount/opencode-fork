...
Key methods can provide an optional `validate` callback. It receives the proposed `Credential.Key`, including form
answers in `configuration`, before the key is saved or selected. Throwing rejects the connection and preserves the
active account; validation does not run when loading stored accounts.
...
await ctx.integration.transform((editor) => {
  editor.method.update({
    ...
    async validate(credential) {
      if (!credential.key.trim()) throw new Error("API key is required")
    },
  })
})
```
