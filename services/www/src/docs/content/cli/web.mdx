...
The QR code and link carry the connection details, so scanning it with a phone
camera opens the OpenCode mobile app when installed and the web app otherwise.
For a server on your local network, the web app offers to open the server's own
web UI instead.
